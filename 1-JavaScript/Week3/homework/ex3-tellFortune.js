...
function tellFortune(numKids, partnerNames, locations, jobTitles) {
  ...
  return `You will be a ${jobTitle} in ${location}, married to ${partnerName} with ${kids} kids.`;
}

function main() {
  const numKids = [1, 2, 3, 4, 5];

  const partnerNames = ['Jane', 'Susan', 'Angelina', 'Marcella', 'Helly'];

  const locations = ['Amsterdam', 'London', 'Paris', 'Madrid', 'Berlin'];

  const jobTitles = ['doctor', 'developer', 'engineer', 'teacher', 'manager'];
